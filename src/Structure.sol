...
contract ClassroomSetup {
    ...
    function enrollStudent(string memory _name, uint256 _age, StudentLevel _level)
        public
        onlyWhenOpen
        validLevel(_level)
    {
        students[msg.sender] =
            Student({name: _name, age: _age, level: _level, isActive: true, enrollmentDate: block.timestamp});
    ...
    function createCourse(string memory _courseName, address _instructor, uint256 _maxStudents)
        public
        onlyPrincipal
        returns (uint256)
    {
